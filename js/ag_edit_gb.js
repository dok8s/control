function SubChk()
{
 ...
 { document.all.winloss_s.focus(); alert("请选择总代理占成数!!"); return false; }
  if(document.all.winloss_a.value=='' )
 { document.all.winloss_a.focus(); alert("请选择代理商占成数!!"); return false; } 
 var winloss_a,winloss_s;
 winloss_s=eval(document.all.winloss_s.value);
 winloss_a=eval(document.all.winloss_a.value); 
 if ((winloss_s+winloss_a) < 120 || (winloss_s+winloss_a) > 150) //ボ羆瞶の瞶坝ぃ眔Θ,きΘ .
 {

 alert(" 总代理及代理商的成数总和须在 5 - 8 成内 , 请重新设定 !! ");
...
function SubChk2(){
 ...
 if(dfwinloss_s!="-" && dfwinloss_a!="-"){
	 ...
	 if(!confirm("预设的成数将在 "+dfday+" 后生效!!确认预设吗?")) {
	  return false;
	 }
 }
