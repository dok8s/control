...
function CheckSTOP(str,chk){
	...
	var page = document.all.page.value;
	if(chk=='Y'){ 
		if(confirm("是否确定启用该大股东?")) document.location=str+"&enable_s="+enable_s+"&page="+page;
	}
	if(chk=='S'){
		if(confirm("是否确定暂停该大股东?")) document.location=str+"&enable_s="+enable_s+"&page="+page;
	}
	if(chk=='N'){
		if(confirm("是否确定停用该大股东?")) document.location=str+"&enable_s="+enable_s+"&page="+page;
	}
}