...
function CheckSTOP(str,chk){
	...
	var page = document.all.page.value;
	if(chk=='Y'){ 
		if(confirm("是否確定啟用該股東?")) document.location=str+"&enable_s="+enable_s+"&page="+page;
	}
	if(chk=='S'){
		if(confirm("是否確定暫停該股東?")) document.location=str+"&enable_s="+enable_s+"&page="+page;
	}
	if(chk=='N'){
		if(confirm("是否確定停用該股東?")) document.location=str+"&enable_s="+enable_s+"&page="+page;
	}
}