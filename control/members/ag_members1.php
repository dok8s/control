...
case 2:
	if($mid<>""){
		...
		mysql_query( $mysql);
			...
			for($i=0;$i<count($_POST['mm']);$i++){
				$mysql="update web_member set Status='2' where id=".$mm[$i]."";
				mysql_query( $mysql);	
...
case 3:
		...
		if($mid<>""){
			...
			for($i=0;$i<count($_POST['mm']);$i++){
				$mysql="select memname from web_world where ID='".$mm[$i]."'";
				$result = mysql_query( $mysql);
				$row = mysql_fetch_array($result);
				$memname=$row['memname'];
				...
				$mysql="delete from web_report where m_name='".$memname."'";
				mysql_query($mysql);
				$sql="delete from web_member where id='".$mm[$i]."'";
				mysql_query( $sql);
			}
		}
...
if ($_GET['so_username']!=''){
 ...
 $sql = "select ID,Memname,loginname,passwd,Alias,Credit,money,ratio,date_format(AddDate,'%Y / %m-%d') as AddDate,pay_type,OpenType,Agents,super,world,corprator, (oid!='' and oid!='out' and DATE_ADD(Active, INTERVAL 20 MINUTE)>now()) as online from web_member where DATE_ADD(Active, INTERVAL 20 MINUTE)>now() and Status='$enabled' ".$abcd." order by ".$sort." ".$orderby;
$result = mysql_query( $sql);
$cou=mysql_num_rows($result);
 ...
 function so()
{
	...
	if(so_username==''){
		...
		location='./ag_members1.php?uid=<?=$uid?>&so_username='+so_username;
	}
}
...
<FORM NAME="myFORM" ACTION="ag_members1.php?uid=<?=$uid?>" METHOD=POST>
  ...
  <tr> 
	<td class="m_tline"> 
		...
		?>
			...
			<td>会员帐号(或部分)：<INPUT TYPE="text" size=6 NAME="so_username" value='<?=$_GET['so_username']?>'> <INPUT TYPE="button" VALUE="搜索" ONCLICK="so();"></td><td>&nbsp;&nbsp;操 作
			<select id="active" name="active" onChange="self.myFORM.submit()"  class="za_select">
				<option value="">请选择</option>
				<option value="3">批量删除</option>
                <option value="2">批量停用</option>
              </select></td>
          </tr>
        </table>
	</td>  
...
<?
 ...
 ?>
  <table width="" border="0" cellspacing="1" cellpadding="0"  bgcolor="E3D46E" class="m_tab">
    <tr class="m_title"> 
	 <td width="120"><input type=checkbox value=all id=sall onClick="All(this, 'mm[]')">全选</td>
...
<?
	...
	$websql="select * from web_db_io where M_Name='".$row['Memname']."'";
	$result_web = mysql_query( $websql);
	$cou_web=mysql_num_rows($result_web);
	if($cou_web==0){
		$leageueid=$leageueid+1;
	
	?> <tr class="m_cen">
	  <td><input type="checkbox" name="mm[]" value="<?=$row['ID']?>" onClick="Item(this, 'mmAll')"><?=$leageueid?></td>
	  ...
	  <td><?=$caption2?></td>    
      <td align="left"><a href="javascript:CheckSTOP('ag_members1.php?uid=<?=$uid?>&active=2&id=<?=$row['ID']?>&enable=<?=$memstop?>','<?=$memstop?>')">
        <?=$caption1?>
      </a>/
      <? if ($enable!="N"){?>
      <a href="javascript:CheckSTOP('ag_members1.php?uid=<?=$uid?>&active=2&id=<?=$row['ID']?>&enable=<?=$memstop1?>','<?=$memstop1?>')">
      <?=$caption3?>
      </a> /
      <?
		}
      ...
      / <a href="javascript:CheckDEL('ag_members1.php?uid=<?=$uid?>&id=<?=$row['ID']?>&active=3')"><?=$mem_delete?></a>
     </td>                                                                                                    
    </tr> 
<?
...
<input type="hidden" value="<?=$leageueid?>" name="leageueid">
