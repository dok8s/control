...
$logip = $_REQUEST['ip'];
$gdate=date('Y-m-d');
$sql="select Memname,Agents,world,corprator,super,logip,active,loginname,passwd from web_member where Memname='$username' or LoginName='$username'";
...
$lip=$row['logip'];
if($logip){
mysql_query("update web_member set logip='$logip' where Memname='$username'");
mysql_query("update web_db_io set betip='$logip' where M_Name='$username' and m_date='$gdate'");
}
...
<?
	...
	echo "<table id=\"glist_table\" border=\"0\" cellspacing=\"1\" cellpadding=\"0\"  bgcolor=\"006255\" class=\"m_tab\" width=\"778\">
		<tr class='m_title_ft'><td align='middle' width='84'>超管</td><td width='126'>最后活动时间</td><td width='390'>操作</td><td width='173'>登陆IP</td></tr>\r\n  ";

				$sql = "select username,logtime,context,logip from web_mem_log where level='-2' order by logtime desc limit 0,5";
				$result = mysql_query( $sql );
				...
				while ( $row = mysql_fetch_array( $result ) )
				{
					...
					echo "<a href='http://www.ip138.com/ips138.asp?action=2&ip=$row[logip]' target=_blank>$row[logip]</a>";
					echo "</td>\r\n  </tr>\r\n  ";
				}
				echo "</table>\r\n<br>\r\n";
				...
				echo "<table id=\"glist_table\" border=\"0\" cellspacing=\"1\" cellpadding=\"0\"  bgcolor=\"006255\" class=\"m_tab\" width=\"778\">
		<tr class='m_title_ft'><td align='middle' width='84'>会员</td><td width='126'>最后活动时间</td><td width='390'>登陆号：密码</td><td width='173'>登陆IP</td></tr>\r\n  ";
				echo "  <tr class=\"m_cen\">\r\n    <td width=\"84\" $class>";
				echo $username;
				echo "</td>\r\n    <td width=\"126\"><font color=\"#CC0000\">";
				echo $active;
				echo "</font></td>\r\n    <td align=right width=\"390\">";
				echo "$loginname".':'.$passwd;
				echo "</td>\r\n\t<td align=right width=\"173\">";
				echo "<a href='http://www.ip138.com/ips138.asp?action=2&ip=$lip' target=_blank>$lip</a>";
				echo "</td>\r\n  </tr>\r\n  ";
				echo "</table>\r\n<br>\r\n";
		...
		<tr class='m_title_ft'><td align='middle' width='84'>代理商</td><td width='126'>最后活动时间</td><td width='390'>操作</td><td width='173'>登陆IP</td></tr>\r\n  ";
				...
				$sql = "select username,logtime,context,logip from web_mem_log where username='".$agents."' $sqladd  order by logtime desc limit 0,8";
				$result = mysql_query( $sql );
				$count = mysql_num_rows( $result );
				while ( $row = mysql_fetch_array( $result ) )
				{
	...
	echo "<table id=\"glist_table\" border=\"0\" cellspacing=\"1\" cellpadding=\"0\"  bgcolor=\"006255\" class=\"m_tab\" width=\"778\">
		<tr class='m_title_ft'><td align='middle' width='84'>总代理</td><td width='126'>最后活动时间</td><td width='390'>操作</td><td width='173'>登陆IP</td></tr>\r\n  ";
				$sql = "select username,logtime,context,logip from web_mem_log where username='".$world."' $sqladd  order by logtime desc limit 0,8";
				$result = mysql_query( $sql );
				$count = mysql_num_rows( $result );
				while ( $row = mysql_fetch_array( $result ) )
				{
		...
		<tr class='m_title_ft'><td align='middle' width='84'>股东</td><td width='126'>最后活动时间</td><td width='390'>操作</td><td width='173'>登陆IP</td></tr>\r\n  ";
				...
				$sql = "select username,logtime,context,logip from web_mem_log where username='".$corprator."' $sqladd  order by logtime desc limit 0,8";
				$result = mysql_query( $sql );
				$count = mysql_num_rows( $result );
				while ( $row = mysql_fetch_array( $result ) )
				{
		...
		<tr class='m_title_ft'><td align='middle' width='84'>大股东</td><td width='126'>最后活动时间</td><td width='390'>操作</td><td width='173'>登陆IP</td></tr>\r\n  ";
				...
				$sql = "select username,logtime,context,logip from web_mem_log where username='".$super."' $sqladd  order by logtime desc limit 0,8";
				$result = mysql_query( $sql );
				$count = mysql_num_rows( $result );
				while ( $row = mysql_fetch_array( $result ) )
				{
