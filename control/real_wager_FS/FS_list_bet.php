...
$sql="select date_format(BetTime,'%m%d%H%i%s')+id as ID,date_format(BetTime,'%H:%i:%s') as BetTime,$bettype as BetType,$middle as Middle,BetScore,M_Result,OpenType,TurnRate,M_Name,LineType from web_db_io where active=6 and linetype=16  ".$search." order by BetTime asc";
$result = mysql_db_query($dbname,$sql);
$cou=mysql_num_rows($result);
if ($cou==0){
	$msg=wterror('δ��Ѱ��ָ���������');
