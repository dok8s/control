  ...
  <table id="glist_table" border="0" cellspacing="1" cellpadding="0"  bgcolor="006255" class="m_tab" width="950">
    ...
    <?
		if($zhd<>"all"){
			switch($gtype){
			default:
				$sql = "select f.mid,f.maxgold,f.m_type,concat(f.m_date,'<br>',lower(substring(DATE_FORMAT(f.m_start,'%h:%i%p'),1,6))) as gdate,f.m_league,f.mb_mid,f.tg_mid,f.mb_team,f.tg_team,f.fopen from foot_match as f join web_db_io as w on find_in_set(f.mid,w.mid)  where f.m_date='".$gdate."' and f.MB_Inball='' group by f.mid order by f.m_start,f.tg_mid,f.mid";
				$gtype='FT';
				break;
			case 'OP':
				$sql = "select o.maxgold,o.m_type,concat(o.m_date,'<br>',lower(substring(DATE_FORMAT(o.m_start,'%h:%i%p'),1,6))) as gdate,o.mid,o.m_league,o.mb_mid,o.tg_mid,o.mb_team,o.tg_team,o.fopen from other_play as o join web_db_io as w on find_in_set(o.mid,w.mid)  where o.m_date='".$gdate."' and o.MB_Inball=''  group by o.mid order by o.m_start,o.tg_mid,o.mid";
				break;
			case 'TN':
				$sql = "select t.maxgold,t.m_type,concat(t.m_date,'<br>',lower(substring(DATE_FORMAT(t.m_start,'%h:%i%p'),1,6))) as gdate,t.mid,t.m_league,t.mb_mid,t.tg_mid,t.mb_team,t.tg_team,fopen from tennis as t  join web_db_io as w on find_in_set(t.mid,w.mid)  where t.m_date='".$gdate."' and t.MB_Inball='' group by t.mid  order by t.m_start,t.tg_mid,t.mid";
				break;
	
			case 'VB':
				$sql = "select v.maxgold,v.m_type,concat(v.m_date,'<br>',lower(substring(DATE_FORMAT(v.m_start,'%h:%i%p'),1,6))) as gdate,v.mid,v.m_league,v.mb_mid,v.tg_mid,v.mb_team,v.tg_team,v.fopen from volleyball as v join web_db_io as w on find_in_set(v.mid,w.mid)  where v.m_date='".$gdate."' and v.MB_Inball='' group by v.mid  order by v.m_start,v.tg_mid,v.mid";
				break;
			case 'BS':
				$sql = "select b.maxgold,b.m_type,concat(b.m_date,'<br>',lower(substring(DATE_FORMAT(b.m_start,'%h:%i%p'),1,6))) as gdate,b.mid,b.m_league,b.mb_mid,b.tg_mid,b.mb_team,b.tg_team,b.fopen from baseball as b join web_db_io as w on find_in_set(b.mid,w.mid)  where b.m_date='".$gdate."' and b.MB_Inball='' group by b.mid  order by b.m_start,b.tg_mid,b.mid";
				break;
			case 'BK':
				$sql = "select b.mid,b.maxgold,concat(b.m_date,'<br>',lower(substring(DATE_FORMAT(b.m_start,'%h:%i%p'),1,6))) as gdate,b.mid,b.m_league,b.mb_mid,b.tg_mid,b.mb_team,b.tg_team,b.fopen from bask_match as b join web_db_io as w on find_in_set(b.mid,w.mid) where b.m_date='".$gdate."' and b.MB_Inball=''  group by b.mid  order by b.m_start,b.m_league,b.mid";
				break;
			case 'FS':
				$sql = "select s.maxgold,s.gtype,s.mid,s.mstart,s.mshow,concat(sleague,'<br>',league) as league,s.team,DATE_FORMAT(s.mstart,'%m-%d<br>%h:%i%p') as gdate,DATE_FORMAT(s.mstart,'%Y-%m-%d') as m_date from sp_match as s join web_db_io as w on find_in_set(s.mid,w.mid)  where DATE_FORMAT(s.mstart,'%m-%d')='$gdate' and s.QQ526738=0 group by s.mid order by s.m_date";
				break;
			}
