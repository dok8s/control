...
if ( $cou == 0 )
{
		...
		$loginfo='查询大股东<font color=red>'.$cid.'</font> : '.$date_start.'至'.$date_end.'<font color=green>'.$pqow.'</font>报表投注明细';
...
$sql = "select corprator as name,count(*) as coun,sum(betscore) as score,sum(m_result) as result,sum(a_result) as a_result,sum(vgold) as vgold,sum(result_a) as result_a,sum(result_s) as result_s,sum(result_c) as result_c from web_db_io where ".$where.( " and super='".$cid."'" );
...
while ( $row = mysql_fetch_array( $result ) )
{
				...
				echo "</td>\r\n    <td>";
				echo $row['coun'];
				echo "</td>\r\n    <td><A HREF=\"report_corprator.php?uid=";
				echo $uid;
				echo "&result_type=";
				echo $result_type;
				echo "&cid=";
