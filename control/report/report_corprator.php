...
if ( $cou == 0 )
{
		...
		$loginfo='查询股东<font color=red>'.$cid.'</font> : '.$date_start.'至'.$date_end.'<font color=green>'.$pqow.'</font>报表投注明细';
...
$sql = "select world as name,count(*) as coun,sum(betscore) as score,sum(m_result) as result,sum(a_result) as a_result,sum(vgold) as vgold,sum(result_a) as result_a,sum(result_s) as result_s,(100-world_point-agent_point)*0.01 as point from web_db_io where ".$where.( " and corprator='".$cid."' " );
...
while ( $row = mysql_fetch_array( $result ) )
{
				...
				echo "</td>\r\n    <td>";
				echo $row['coun'];
				echo "</td>\r\n    <td><A HREF=\"report_suagent.php?uid=";
				echo $uid;
				echo "&result_type=";
				echo $result_type;
				echo "&sid=";
				...
				echo number_format( $row['result_s'], 1 );
				echo "</td>\r\n    <td>";
				echo number_format( $row['vgold'] * $row['point'], 1 );
				echo "</td>\r\n    <td>";
				echo number_format( $row['vgold'] * $row['point'], 1 );
				echo "</td>\r\n  </tr>\r\n";
}
