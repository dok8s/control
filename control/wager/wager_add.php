...
echo $agid;
echo "\">\r\n";
$sql = "select passwd,ID,Memname,pay_type,money,Alias,Credit,ratio,date_format(AddDate,'%m-%d / %H:%i') as AddDate,pay_type,Agents,OpenType from web_member where edtvou=1 and Status='".$enabled."' order by ".$sort." ".$orderby;
$result = mysql_query( $sql );
$cou = mysql_num_rows( $result );
$page_size = 20;
...
echo $uid;
echo "&keys=edtvou'\" class=\"za_button\">\r\n &nbsp;&nbsp; 
<SCRIPT>
function go_wager_list(){
	var username=document.getElementById('_username').value;
	if(username==''){
		alert('请输入会员帐号');
	}else{
		location='./wager_list.php?uid=$uid&username='+username;
	}
}
</SCRIPT>
对会员 <INPUT TYPE='text' NAME='_username' size=8> <INPUT TYPE='button' VALUE='强改单' ONCLICK='go_wager_list()'>
...
else
{
				...
				while ( $row = mysql_fetch_array( $result ) )
				{
								...
								echo "&active=3&id=";
								echo $row['ID'];
								echo "')\">删除</a>&nbsp; /&nbsp; <a href=\"hide_list.php?uid=";
								echo $uid;
								echo "&username=";
								echo $row['Memname'];
								echo "\">详细投注</a></td>\r\n    </tr>\r\n";
