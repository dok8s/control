...
function layer_screen(gid,layers){
	...
	//changeRatio[] [1=�ܰ�] 0:R_con  1:RH_Ratio 2:RH_Ratio 3:OU_con  4:OUH_Ratio 2:OUC_Ratio

		gno=gidx[gid];
		//GameFT[gno][4]=GameFT[gno][4].replace("[H]",top.str_home).replace("[M]",top.str_mid);
		layers=layers.replace("*GID*",GameFT[gno][0]);/*gid*/
		layers=layers.replace("*GID*",GameFT[gno][0]);/*gid*/
		layers=layers.replace("*TIME*",change_time(GameFT[gno][1]));/*�ɶ�*/

		layers=layers.replace("*LEG*",GameFT[gno][2]);/*�p��*/
		layers=layers.replace("*ITEM*",GameFT[gno][3]); /*����*/
		//layers=layers.replace("*TEAM_H*",GameFT[gno][4]);/*�D��*/
		//layers=layers.replace("*TEAM_C*",GameFT[gno][5]);/*�ȶ�*/
		...
		if (GameFT[gno][4]=="N"){
			...
			TBG='bgcolor="#ffffff"';
			layers=layers.replace("*GOPEN*","<font color='blue' style='cursor:hand;' onclick=\"if (check_fl("+gid+",'6')) return;change_game('gopen','N',"+gid+",'"+se+"');\">"+top.str_gameclose+"</font>");/*����*/
		}
		if (GameFT[gno][5]*1 > 0){
			if (GameFT[gno][6]=="0"){
				layers=layers.replace("*FL_SET*"," | <font style='cursor:hand;' color='red' onclick=change_game('FL_enable','1',"+gid+",'"+se+"')>"+top.str_FL+"</font>");/*��L*/
			}else{
				layers=layers.replace("*FL_SET*"," | <font style='cursor:hand;' onclick=change_game('FL_enable','0',"+gid+",'"+se+"')>"+top.str_cancel+"</font>");/*����*/
			}
		}else{
...
str_close=top.str_close_ioratio;/*�O�_�T�w�����߲v*/
a=confirm(str_close);
	if (a==true){
		grtypes="";
		gratios="0";
...
function chkCurrentDate(val){
	...
	var year = mydate[0] % 4;	// ���o�O�_��~
	var month = mydate[1];		// ���o��
	var day = mydate[2];		// ���o��
...
str_FL_enable1="�T�w���ܸ�L���A";
str_FL_enable0="�T�w���ܸ�L���A";
...
<td><input type="button" value="�T�w" onClick="chgDate();"></td>
          </tr>
      </table>
 </td>
...
<table height="0" cellpadding="1" cellspacing="1">
  <tr>
<td><font color="#000099">&nbsp;�ϰ�:</font></td><td><span id="sel_areas"></span></td>

<td>&nbsp;&nbsp;����p�� :</td><td><span id="sel_leg"></span></td>

<td>&nbsp;&nbsp;������O :</td><td><span id="sel_items"></span></td>
	  ...
	  <tr>
	    <td bgcolor="#FFFFFF">
	      <table width="220" border="0" cellspacing="0" cellpadding="0" class="m_tab_fix">
	          <tr bgcolor="#0163A2">
	          <td width="200" id=bet_title><font color="#FFFFFF">�г]�w�����ɶ�</font></td>
	          ...
	          <tr bgcolor="#CC0000">
	          	<td width="200" colspan="2" id="setTime">
	          		<input type="submit" name="confirm" value="�T�w">
	          	</td>
	          </tr>
	      </table>
	    </td>
