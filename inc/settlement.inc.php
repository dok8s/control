...
function settlement($id, $mb_inball, $tg_inball, $matche_status=0, $more=array(),$ball1)
{
	...
	$cancel=0;
	$status=0;
	if($matche_status>0 and ($mb_inball=='-1' or $tg_inball=='-1')){
	...
	if ($notgraded==1){
		...
		if($row['danger']==2){
			$sql="update web_db_io set status='$status',QQ526738='$ball',vgold=0,m_result=0,a_result=0,result_a=0,result_s=0,result_c=0,result_type=1,Q10000='$ball1' where id=".$row['id'];
		}else if($row['danger']==1){
			$sql="update web_db_io set status='$status',danger=3,QQ526738='$ball',vgold=$gold_d,m_result=$members,a_result=$agents,result_a=$result_a,result_s=$result_s,result_c=$result_c,result_type=1,Q10000='$ball1' where id=".$row['id'];
		}else{
			$sql="update web_db_io set status='$status',QQ526738='$ball',vgold=$gold_d,m_result=$members,a_result=$agents,result_a=$result_a,result_s=$result_s,result_c=$result_c,result_type=1,Q10000='$ball1' where id=".$row['id'];
		}
...
function settlement_cancel($id){
	...
	if($row['id']==$id && $row['pay_type']==1 && ($row['m_result']!=0 || $row['result_type']!=0)){
		$sql = "update web_db_io set QQ526738='',vgold=0,m_result=0,a_result=0,result_a=0,result_s=0,result_c=0,result_type=0,Q10000='' where id='$id' ";
